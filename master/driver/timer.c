...
void TIM3_IRQHandler(void)
{
   	if(TIM3->SR&0X0001){
			...
			//陀螺仪数据
			GetGyroRaw();
			DegTransform(&gyroButterworthData,&Mpu.deg_s);
			RadTransform(&gyroButterworthData,&Mpu.rad_s);

			//加速度数据
			GetAccRaw();

			//姿态解算时加速度低通滤波
			acc_iir_lpf(&accButterworthData,&acc_att_lpf,Mpu.att_acc_factor);
			AccDataTransToG(&acc_att_lpf,&Mpu.acc_g);

			//高度融合加速度低通滤波 z
			acc_iir_lpf(&accButterworthData,&acc_fix_lpf,Mpu.fix_acc_factor);

			//气压计数据
			//get_spl_status();

			//姿态解算
			IMUUpdate(Mpu.rad_s.x,Mpu.rad_s.y,Mpu.rad_s.z,Mpu.acc_g.x,Mpu.acc_g.y,Mpu.acc_g.z);
